...
EXCLUDE_DIRS=(
    ...
    "uploads"
    "unprocessed"
    "data" # Be specific if needed, e.g., "backend/data"
)
...
main() {
    ...
    while IFS= read -r file; do
        ...
        if [[ $size -gt $max_size_bytes ]]; then
            echo -e "${C_GRAY}[$file_count/$total_files] Skipping large file: $file ($((size/1024)) KB)${C_RESET}"
            continue
        fi

        echo -e "${C_GREEN}[$file_count/$total_files]${C_RESET} Adding ${C_GRAY}$file${C_RESET}"
